...
const ReviewReportList = () => {
    ...
    const fetchReviewReports = useCallback(async () => {
        ...
        try {
            ...
            console.log("Fetched review reports:", response.data); // 디버깅용 로그
        } catch (err) {
    ...
    if (loading) return <p>리뷰 신고 목록을 불러오는 중...</p>;
    if (error) return <p className="error-message">{error}</p>;

    return (
        <div className="management-section">
            <h2 className="section-title">리뷰 신고 목록</h2>
            ...
            <div className="table-wrapper">
                <table>
                    ...
                    <tbody>
                        {reports.length === 0 ? (
                            <tr>
                                <td colSpan="8" className="no-data">조건에 맞는 신고된 리뷰가 없습니다.</td>
                            </tr>
                        ) : (
                            reports.map((r) => (
                                <tr key={r.reportId}>
                                    ...
                                    <td>{r.reportDate ? new Date(r.reportDate).toLocaleDateString() : 'N/A'}</td>
            ...
            {isDetailModalOpen && selectedReport && (
                <ReportDetailModal
                    ...
                    reportType="review" // 이 모달이 리뷰 신고용임을 명시
                    onClose={closeReportDetailModal}
                    getReportStatusText={getReportStatusText}
                    fetchReports={fetchReviewReports} // 모달 내에서 처리 후 목록 새로고침
                />
            )}
        </div>
    );
