...
const EditRestaurant = ({ restaurantData, onSave, onCancel }) => {
    ...
    useEffect(() => {
        const fetchStadiums = async () => {
            try {
                ...
                alert("구장 목록을 불러오는 데 실패했습니다. 잠시 후 다시 시도해주세요.");
            }
        };
    ...
    const handleSave = async () => {
        ...
        if (typeof id === 'undefined' || id === null) {
            alert("식당 정보를 불러오지 못했거나 식당 ID가 없습니다. 페이지를 새로고침하거나 관리자에게 문의하세요.");
            return; // Stop save logic if ID is invalid
        }

        // --- 2. 필수 정보 유효성 검사 ---
        if (!restaurantName.trim() || !stadiumName.trim() || restaurantResvStatus === '') {
            alert('필수 정보를 모두 입력해주세요 (음식점 이름, 구장 선택, 예약 가능 여부).');
            return;
        }
        ...
        try {
            ...
            alert(`식당 정보 수정에 실패했습니다: ${error.response ? error.response.data.message || error.response.statusText || '서버 오류' : error.message}`);
        }
    };
    ...
    return (
        <div className="edit-restaurant-page-container">
            <h1 className="page-title">음식점 정보 수정</h1>

            <div className="image-upload-section">
                <div className="restaurant-image-placeholder-edit">
                    {imagePreviewUrl ? (
                        <img src={imagePreviewUrl} alt="음식점 이미지 미리보기" className="image-preview" />
                    ) : (
                        <p>음식점 이미지를 추가해주세요</p>
                    )}
                </div>
                ...
                <div className="image-upload-controls">
                    <input
                        ...
                        style={{ display: 'none' }}
                    />
                    <button type="button" className="find-file-button" onClick={handleFileButtonClick}>
                        파일 찾기
                    </button>
                </div>
            </div>
            ...
            <div className="form-section">
                <div className="form-group">
                    <label htmlFor="restaurant-name" className="form-label">음식점 이름</label>
                    <input
                        type="text"
                        id="restaurant-name"
                ...
                {restaurantInsideFlag === 1 ? ( // 구장 외부 식당 필드
                    <>
                        <div className="form-group">
                            <label htmlFor="phone-number" className="form-label">식당 전화번호</label>
                            <input
                                type="text"
                                id="phone-number"
                                className="form-input"
                        ...
                        <div className="form-group">
                            <label htmlFor="basic-address" className="form-label">기본 주소</label>
                            <input
                                type="text"
                                id="basic-address"
                                className="form-input"
                        ...
                        <div className="form-group">
                            <label htmlFor="detail-address" className="form-label">상세 주소</label>
                            <input
                                type="text"
                                id="detail-address"
                                className="form-input"
                        ...
                        <div className="form-group">
                            <label htmlFor="opening-hours" className="form-label">운영 시간</label>
                            <input
                                type="text"
                                id="opening-hours"
                                className="form-input"
                        ...
                        <div className="form-group">
                            <label htmlFor="last-order-time" className="form-label">라스트 오더 시간</label>
                            <input
                                type="text"
                                id="last-order-time"
                                className="form-input"
                        ...
                        <div className="form-group">
                            <label htmlFor="break-time" className="form-label">브레이크 타임</label>
                            <input
                                type="text"
                                id="break-time"
                                className="form-input"
                        ...
                        <div className="form-group">
                            <label htmlFor="closing-days" className="form-label">휴무일</label>
                            <input
                                type="text"
                                id="closing-days"
                                className="form-input"
                    ...
                    <>
                        <div className="form-group">
                            ...
                            <label htmlFor="zone-name" className="form-label">구역</label>
                            <input
                                type="text"
                                id="zone-name"
                                className="form-input"
                        ...
                        <div className="form-group">
                            ...
                            <label htmlFor="restaurant-location" className="form-label">상세 구역</label>
                            <input
                                type="text"
                ...
                <div className="form-group radio-group">
                    <label className="form-label">예약 가능 여부</label>
